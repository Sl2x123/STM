...
def delete_project(db: Session, project_id: int):
    ...
    # Clean up all dependencies referencing projects.id
    db.query(ProjectTask).filter(ProjectTask.project_id == project_id, ProjectTask.parent_id != None).delete(synchronize_session=False)
    db.query(ProjectTask).filter(ProjectTask.project_id == project_id).delete(synchronize_session=False)
    db.query(Blogger).filter(Blogger.project_id == project_id).delete(synchronize_session=False)
    db.query(Company).filter(Company.project_id == project_id).delete(synchronize_session=False)
    db.query(RnpItem).filter(RnpItem.project_id == project_id).delete(synchronize_session=False)
